...
<img src="assets/agent-context-window-hero.png" alt="agent-context-window — Vedic Arsenal by Darshankumar Joshi" width="100%" />
...
```mermaid
flowchart LR
    A[Input] --> B[agent-context-window]
    B --> C{Process}
    C -- Success --> D[Output]
    C -- Error --> E[Handle / Retry]
    E --> B
    style B fill:#6b21a8,color:#fff
    note["Context Window — Zero Dependencies"]
```
